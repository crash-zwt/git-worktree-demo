...
- **修改方向**：简述（效能、修复、样式等）
- **内容**：列出具体修改点，**禁止**出现任何文件路径（包括相对路径），一律改用功能描述，例如"新增手风琴展开动画"而非"修改 `src/components/FAQ.jsx`"
...
将完整的 PR Title + Description 以 **markdown code block** 的形式输出，让用户可以直接复制贴上到 GitHub PR。
...
- 用户可要求调整任何部分后再复制使用
- **重要**：Description 中**禁止**出现任何文件路径（包括相对路径），一律改用纯功能描述。
